...
<!-- 2. 一个监听小助手，专门负责在评论区出现后告诉 MathJax 开工 -->
<script>
  function listenForGiscusAndRenderMath() {
    const observer = new MutationObserver(function(mutations, obs) {
      // 查找 giscus 创建的 iframe 元素
      const giscusIframe = document.querySelector('.giscus-frame');
      if (giscusIframe) {
        console.log("检测到 giscus iframe，准备渲染公式。");
        if (typeof MathJax !== 'undefined') {
          // 关键一步：让 MathJax 重新扫描并渲染整个页面
          MathJax.typesetPromise().catch(function(err) {
            console.warn("MathJax 渲染时出错：", err);
          });
        } else {
          console.warn("未找到 MathJax，请检查是否正确加载。");
        }
        // 找到目标后，停止观察，避免重复执行
        obs.disconnect();
      }
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', listenForGiscusAndRenderMath);
  } else {
    listenForGiscusAndRenderMath();
  }
</script>
